...
//     d3.selectAll("svg > *").data([]).exit().remove();
        ...
        var xAxis_b = d3.svg.axis().scale(x_b).ticks(30,"s").orient("bottom").tickFormat(d3.format("04d")).tickValues([1971,1975,1980,1985,1990,1995,2000,2005,2010,2015,2020,2025,2030,2035,2040]).tickSize(10,0);
    ...
    2026,2027,2028,2029,2030,2031,2032,2033,2034,2035,2036,2037,2038,2039,2040];
     ...
     var mycolor = ['#deebf7','#9ecae1','#3182bd','#e5f5f9','#99d8c9','#2ca25f','#fee6ce','#fdae6b','#e6550d','#ffabab','#f86161','#9b0707'];
        ...
        d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("Units: Primary Energy Consumption (Quadrillion BTUs)");
    ...
    var fieldsById = d3.nest().key(function(d) {return d.id;})
            ...
            var radio_button = ["Absolute Value","Per Capita"],
            j = 0;

            var form = d3.select("#radio_button_container").append("form");
            ...
            radios0.onclick = function(){
                if (field.id == "pepercapita" && radios0.checked){
                    d3.selectAll("#Summation").remove();
                    field = fields[0];
                    location.hash = "#" + [field.id, year].join("/");
                    colors = mycolor_blue.map(function(rgb) { return d3.hsl(rgb); });
                    d3.select("#map-container > svg > text").remove();
                    d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("Primary Energy Consumption (Units: Quadrillion BTUs)");
                    ...
                    d3.select("#map-container > svg > text").remove();
                    d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("CO2 Emission (Units: Billion Metric Tonnes)");
                    ...
                    d3.select("#map-container > svg > text").remove();
                    d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("GDP PPP (Units: Trillion 2005 US Dollar)");
            };
        };
            ...
            radios1.onclick = function(){
                if (field.id == "PECons" && radios1.checked){
                    ...
                    colors = mycolor_blue.map(function(rgb) { return d3.hsl(rgb); });
                    d3.select("#map-container > svg > text").remove();
                    d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("Primary Energy Consumption Per Capita (Units: Million BTUs)");
                    addSummation();
                    ...
                    colors = mycolor_green.map(function(rgb) { return d3.hsl(rgb); });
                    d3.select("#map-container > svg > text").remove();
                    d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("CO2 Emission Per Capita (Units: Metric Tonnes)");
                    addSummation();
                    ...
                    colors = mycolor_orange.map(function(rgb) { return d3.hsl(rgb); });
                    d3.select("#map-container > svg > text").remove();
                    d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("GDP PPP Per Capita (Units: Thousand 2005 US Dollar)");
                    addSummation();
                };
            };
    ...
    var populationSelect = d3.select('.button.-red').on("click",function(e){
        ...
        d3.select("#map-container > svg > text").remove();
        d3.select("#map-container > svg").append("text").attr("x",5).attr("y","88%").attr("dy",".35em").text("Population (Units: Million)");
    ...
    var proj = d3.geo.eckert3(),
      ...
      var segmentized = location.search === "?segmentized",
        url = ["data",
            segmentized ? "world-segmentized.topojson" : "world-220m_withoutAntarctica.json"
        ].join("/");

d3.json(url, function(topo) {
        topology = topo;
        geometries = topology.objects.countries.geometries;
    d3.csv("data/All_final_standard20160208.csv", function(data) {
             rawData = data;
            dataById = d3.nest()
    ...
    // update the data
         countries.data(features)
                .select("title")
                .text(function(d) {
                    return [d.properties.country, fmt(value(d))==0 ? "NA" : value(d).toFixed(2)].join(": ");
                });
...
function addColorBar(){
    d3.selectAll("#colorbar").remove();
        ...
        var colorbar = Colorbar()
                    .origin([25,540])
                    .thickness(100)
                    .scale(fillings).barlength(350).thickness(10)
                    .orient(orientation);
...
function addSummation(){
    ...
    var parts2 = location.hash.substr(1).split("/"),
        desiredFieldId2 = parts2[0];
            
        field2 = fieldsById[desiredFieldId2];
        year2 = d3.select("input[type=range]")[0][0].value;

    var variables = [field2.id, year2].join("");
    var fmtSum = Number(dataById["900"][variables]).toFixed(2)==0.00 ? "NA" : Number(dataById["900"][variables]).toFixed(0);

        d3.selectAll("#Summation").remove();
        d3.selectAll(".summation_units").remove();
        d3.select("#map-container > svg")
                    .append("text")
                    .attr("id","Summation").attr("x","74%")
                    .attr("y","90%")
                    .text("Global Total: " +fmtSum);

    if (field2.id=="PECons"){
        d3.select("#map-container > svg").append("text").attr("class","summation_units").attr("x","87%").attr("y","90%").text("Quadrillion BTUs");
    }else if (field2.id=="Carbon"){
        d3.select("#map-container > svg").append("text").attr("class","summation_units").attr("x","87%").attr("y","90%").text("Billion Tonnes");
    }else if (field2.id=="GDP"){
        d3.select("#map-container > svg").append("text").attr("class","summation_units").attr("x","87%").attr("y","90%").text("Trillion Dollars");
    }else if (field2.id=="Population"){
        d3.select("#map-container > svg").append("text").attr("class","summation_units").attr("x","88%").attr("y","90%").text("Million People");
    };

        if((radios1.checked == true)&&(field2.id!="Population")){
            d3.selectAll("#Summation").remove();
            d3.selectAll(".summation_units").remove();
        };
