...
var fmtSum = Number(dataById["900"][variables]).toFixed(2)==0.00 ? "NA" : Number(dataById["900"][variables]).toFixed(2);

        d3.selectAll("#Summation").remove();
        d3.select("#map-container > svg")
                    .append("text")
                    .attr("id","Summation").attr("x","85%")
                    .attr("y","90%")
                    .text("Sum: " +fmtSum);
};
...
function filterCSV(csvfile, key, value) {
    var result = [];
    for (var indicator in csvfile) {
        if (csvfile[indicator][key] === value) {
            result.push(csvfile[indicator]);
        }
    }
    return result;
};